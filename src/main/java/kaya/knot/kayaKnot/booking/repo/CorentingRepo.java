...
public interface CorentingRepo extends JpaRepository<Corenting,String> {
    ...
    @Query("SELECT new kaya.knot.kayaKnot.booking.entity.dto.CorentingUserHouseDTO(a.id,a.renterStatus,a.renterComment,u.id,hs.id,h.id) FROM Corenting a JOIN Users u  ON a.renterId.id=u.id JOIN HouseStatus hs ON a.houseStatusId.id=hs.id JOIN House h ON a.houseStatusId.houseId.id=h.id and a.houseStatusId.id=:status_id")
